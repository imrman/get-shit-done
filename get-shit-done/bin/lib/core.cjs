...
/**
 * Remove linked worktrees whose branches are already merged into HEAD.
 *
 * Safety constraints:
 *  - Never removes the main worktree.
 *  - Never removes the process cwd or an ancestor of cwd.
...
function pruneOrphanedWorktrees(repoRoot) {
  ...
  try {
    ...
    // First entry is the main worktree — never touch it.
    for (let i = 1; i < worktrees.length; i++) {
      ...
      if (ancestorCheck.exitCode !== 0) continue;
