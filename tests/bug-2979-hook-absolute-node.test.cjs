...
function parseHookCommand(cmd) {
  const runnerMatch = cmd.match(/^("[^"]+"|\S+)\s+(.+?)\s*$/);
  if (!runnerMatch) {
    return { runner: null, hookPath: null, raw: cmd };
  }
  const runner = runnerMatch[1];
  const pathExpr = runnerMatch[2];

  let hookPath = null;
  const portable = pathExpr.match(/^"\$HOME"'([^']*)'$/);
  const singleQuoted = pathExpr.match(/^'([^']*)'$/);
  const doubleQuoted = pathExpr.match(/^"([^"]*)"$/);
  if (portable) hookPath = '$HOME' + portable[1];
  else if (singleQuoted) hookPath = singleQuoted[1];
  else if (doubleQuoted) hookPath = doubleQuoted[1];

  return { runner, hookPath, raw: cmd };
}
