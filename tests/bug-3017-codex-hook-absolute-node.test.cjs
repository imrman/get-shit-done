...
describe('Bug #3017: buildCodexHookBlock emits absolute node runner', () => {
  ...
  test('integrates with resolveNodeRunner() in the live process — runner equals process.execPath (#3022 CR)', () => {
    ...
    // stripping toml + JSON escape layers) must be exactly the path
    // resolveNodeRunner selected. That helper may canonicalize a volatile
    // Homebrew Cellar path to the stable bin symlink.
    const expected = runner.slice(1, -1);
